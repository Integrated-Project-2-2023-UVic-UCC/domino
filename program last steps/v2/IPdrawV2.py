...
ser = serial.Serial('COM3', 9600, timeout=.1)  #change the com3 to the bluetooth serial port
ser.read_all()
...
while True:
    ...
    for angle,distance in zip(angles,distancias):

        buff = b''
        buff += angle.to_bytes(1, byteorder='little')
        buff += distance.to_bytes(2, byteorder='little')
        ser.write(buff)
        ser.flush()
